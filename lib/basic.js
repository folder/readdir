...
const readdir = async (basedir, options = {}) => {
  ...
  const isIgnoredFile = opts.isIgnoredFile || noop(false);
  const isIgnoredDir = opts.isIgnoredDir || noop(false);
  ...
  let recurse = opts.recursive === true || (typeof options.depth === 'number' && options.depth > 1);

  // const keep = file => opts.keep !== false && file.keep !== false && file.path !== cwd;
